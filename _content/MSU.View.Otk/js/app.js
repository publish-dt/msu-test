...
function countdown() {
    ...
    var today = 0;
    var thour = !showBlock ? 0 : Math.floor(timeleft / 3600);
    var tmin = !showBlock ? 0 : Math.floor(timeleft / 60 - 60 * thour);
    var tsec = !showBlock ? 0 : timeleft % 60;
    ...
    if (!showBlock && countStartPosyl >= (301 + rndCounter)) { // через 5+rnd минут после начала Посыла синхронизируем (получаем остаток времени до следующего Посыла)
...
document.body.addEventListener('htmx:configRequest', function (evt) {
    ...
    if (isAutonomyOrStatic() ||  // это для автономного или статического режима
        (evt.detail.triggeringEvent && evt.detail.triggeringEvent.detail
            && evt.detail.triggeringEvent.detail.notfound === true) || // или если при предыдущей попытке не найден
        ...
        path = detail.path;
        if (!isAutonomyOrStatic() && detail.triggeringEvent && detail.triggeringEvent.type === triggerOnload) { // detail.headers["HX-Trigger"] === "main-cont"
    ...
    // подставляем .spa, при необходимости
    detail.path = replaceExt(detail, url); /*(sendExtSPA
        && (detail.boosted
            || (detail.triggeringEvent && detail.triggeringEvent.type === triggerOnload))
        && url.href.indexOf('.spa') === -1) ? (url.href.replace(".html", '') + (url.pathname === basePath ? "index" : "") + ".spa") : url.href;*/

    if (detail.triggeringEvent && detail.triggeringEvent.detail && detail.triggeringEvent.detail.replaceEndPath)
        detail.path = detail.path.substring(0, detail.path.lastIndexOf('/') + 1) + detail.triggeringEvent.detail.replaceEndPath;

    if (detail.triggeringEvent && detail.triggeringEvent.detail && detail.triggeringEvent.detail.headers) {
...
function reCallRequest(evt, withoutBase) {
    if (hostname !== "") {
        ...
        let path = replaceExt(detail, url); // (sendExtSPA && evt.detail.boosted && url.href.indexOf('.spa') === -1) ? (url.href.replace(".html", '') + (url.pathname === basePath ? "index" : "") + ".spa") : url.href;
...
function replaceExt(detail, url) {
    return (sendExtSPA
        ...
        && url.href.indexOf('.spa') === -1
    )

        ? (url.href.replace(".html", '') + (url.pathname === basePath ? "index" : "") + ".spa") : url.href;
}
